...
Packet ServerReceive(int sockfd, struct sockaddr *address, socklen_t *addrlen, int seqnum) {
    ...
    while (1) {
        ...
        printf("Received: ");
        logPacket(packet);
        if (packet.header.cksum != getChecksum(packet))  //******STUDENTWORK******)
        {
            ...
            //******STUDENT WORK******);
            ServerSend(sockfd, address, *addrlen, !seqnum);  //******STUDENTWORK******
        } else {
            printf("Good packet\n");
            //******STUDENT WORK******
            ServerSend(sockfd, address, *addrlen, seqnum);
...
int main(int argc, char *argv[]) {
    ...
    // No because we need to use the client side
    //******STUDENT WORK******
    ...
    do {
        packet = ServerReceive(sockfd, (struct sockaddr *)&clientaddr,
                               &clientaddr_len, seqnum);  //******STUDENT WORK****** );
        //******STUDENT WORK******
