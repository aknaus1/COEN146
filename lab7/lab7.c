...
* Description: This program devlops a link state routing algorithm  
*/
...
int main(int argc, char *argv[]) {
    ...
    // get nodes
    if ((myid = atoi(argv[1])) >= N) { //argv[1] = router_id
        printf("Bad ID\n");
        exit(1);
    }

    //get costs
    if ((fp = fopen(argv[2], "r")) == NULL) { //argv[2] = costs_file
        printf("can't open %s\n", argv[3]);
        return 1;
    }
    ...
    // get machine info
    if ((fp = fopen(argv[3], "r")) == NULL) { //arg[3] = machine_file
        printf("can't open %s\n", argv[4]);
        return 1;
    }
    ...
    for (i = 0; i < 3; i++) {
        printf("New cost for router (<neighbor id> <cost>):\n");
        scanf("%d%d", &id, &cost);
        if (id >= N || id == myid) {
            printf("wrong id\n");
            break;
