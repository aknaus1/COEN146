...
* Date: 2/15/2021
* Title: Lab6 - server
* Description: This program is the server side code
*/
#include "lab6.h"
...
Packet serverReceive(int sockfd, struct sockaddr *servAddr, socklen_t *addrlen, int seqnum)
{
    ...
    while (1) {
        ...
        if(packet.header.len != strlen(packet.data)){
            printf("Bad packet length, expected %d, actual %lu \n", packet.header.len, strlen(packet.data));
...
int main(int argc, char *argv[]) {
    ...
    servAddr.sin_port = htons(atoi(argv[1])); //Port argv[1] is assigned
    servAddr.sin_addr.s_addr = INADDR_ANY;

    // bind the socket
    //******STUDENT WORK******
    if((bind(sockfd, (struct sockaddr *)&servAddr, sizeof(struct sockaddr))) < 0){
        perror("Failure to bind server servAddr to the endpoint socket \n");
        exit(1);
    }
